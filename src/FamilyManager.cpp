...
void oneTest() {
  ...
  typedef FamilyMember FamilyMember;
  ...
  if (t_int_sex == 0)
    t_sex = FamilyTree::Sex::girl;
  else if (t_int_sex == 1)
    t_sex = FamilyTree::Sex::boy;
  else
    t_sex = FamilyTree::Sex::unknown;
  ...
  while (cin >> t_insert >> t_name >> t_int_sex >> t_age >> t_hus_mot_name) {
    ...
    auto hus_mot =
        familytree.queryMember([&](const FamilyMember *person) -> bool {
